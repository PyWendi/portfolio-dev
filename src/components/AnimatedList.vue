<template>
  <div :class="`scroll-list-container relative w-full ${className}`.trim()">
    <div
      ref="listRef"
      :class="[
      ...
      >
        <slot :item="item" :index="index" :is-selected="selectedIndex === index">
          <div :class="['p-4 bg-[#222] rounded-lg mb-4', selectedIndex === index ? 'bg-[#222]' : '', itemClassName]">
            <p class="m-0 text-white">{{ typeof item === 'string' ? item : item.title }}</p>
          </div>
        </slot>
      </AnimatedItem>
    </div>
...
const emit = defineEmits<{
  itemSelected: [item: string | TimelineItem, index: number];
}>();
...
const handleItemClick = (item: string | TimelineItem, index: number) => {
  selectedIndex.value = index;
...
const handleKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'ArrowDown' || (e.key === 'Tab' && !e.shiftKey)) {
    ...
    if (selectedIndex.value >= 0 && selectedIndex.value < props.items.length) {
      e.preventDefault();
      emit('itemSelected', props.items[selectedIndex.value] as string | TimelineItem, selectedIndex.value);
    }o
  }
};
